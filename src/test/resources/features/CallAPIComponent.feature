Feature: Call API Component WorkFlow Feature
  ...
  @CreateNewCallAPIWorkFlowAndPublish @RegressionTest
  Scenario Outline: Create a new CallAPI Component Workflow and Publish
    ...
    Examples:
      |  |  | labelName | configureStep  | variableName   | placeholderValue | helptextValue | dataSource1 | defaultValue                        | dataSource | workflowName | position | workflowDescription | status  | requestType |  |  |  |
      |  |  | KeyLBL    | Configure Step | endpointurlget | KeyName          | HelpKeyTest   | Call API    | https://api.restful-api.dev/objects | Input      | CallAPIGET   | 1        | CallAPIGET Test     | Version | GET         |  |  |  |
